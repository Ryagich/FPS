    ...
    public class BannerYG : MonoBehaviour
    {
        ...
        public void RecalculateRect()
        {
            if (CheckDevice())
            {
                ...
                var left = rt.localPosition.x;
                var top = -rt.localPosition.y;

                if (scaler.uiScaleMode == CanvasScaler.ScaleMode.ScaleWithScreenSize)
                {
                    var multResolution = new Vector2(Screen.width / scaler.referenceResolution.x, Screen.height / scaler.referenceResolution.y);
                ...
                var _width = width.ToString().Replace(",", ".") + "%";
                var _height = height.ToString().Replace(",", ".") + "%";
                var _left = left.ToString().Replace(",", ".") + "%";
                var _top = top.ToString().Replace(",", ".") + "%";
