...
public class AttackAction : Action
{
        ...
        Vector3 hitNormal = default, bool organic = false, Transform target = null)
    {
        ...
        else if (target && organic)
        {
            ...
            var targetHealthInParent = target.GetComponentInParent<HealthManager>();
            if (targetHealth)
            {
                targetHealth.TakeDamage(hitPoint, direction, controller.classStats.bulletDamage,false,
                    target.GetComponent<Collider>(), controller.gameObject);
            }
            else if (targetHealthInParent)
            {
                targetHealthInParent.TakeDamage(hitPoint, direction, controller.classStats.bulletDamage,false,
                    target.GetComponent<Collider>(), controller.gameObject);
            }
