...
namespace InfimaGames.LowPolyShooterPack.Legacy
{
    public class ProjectileScript : MonoBehaviour
    {
        ...
        [Tooltip("Explosion intensity")] public float power = 250.0F;

        [Header("Rocket Launcher Projectile")] [Tooltip("Enabled if the projectile has particle effects")]
        ...
        private void OnCollisionEnter(Collision collision)
        {
            ...
            foreach (var с in colliders)
            {
                ...
                if (с.tag == "Target" &&
                    с.GetComponent<TargetScript>().isHit == false)
                {
                    //Toggle the isHit bool on the target object
                    с.gameObject.GetComponent<TargetScript>().isHit = true;
                }

                if (с.tag == "ExplosiveBarrel")
                {
                    с.transform.gameObject.GetComponent<ExplosiveBarrelScript>().explode = true;
                }
